import { View, Text, SafeAreaView, ActivityIndicator, ScrollView, TextInput, RefreshControl } from 'react-native'
  ...
  const handleUpdate = async () => {
    if (!name) {
      setErrorName(true);
        ...
        try {
            setIsLoading(true);
            const res = await axiosClient(`${BASE_URL}/v1/user/update/${user?.info?._id}`,
              {
                method: 'post',
                data: formData
              }
      ...
      
    {
      ...
      >
        <View className='px-[14px] pb-[40px] mt-[10px]'>
            <Text className='text-[20px] font-psemibold'>Thông tin cá nhân</Text>
                ...
                <View className='mt-[10px] border-b-[1px] border-solid border-[#d3cece]'>
                    ...
                    <View className='flex-row justify-center items-center'>
                        <View className='flex-1'>
                            <TextInput value={phone} onChangeText={val => {setErrorPhone(false); setPhone(val)}} className='text-[16px]' selectionColor="#333" />
                        </View>
                        {
                            errorPhone &&
                            <Feather className='' name="alert-circle" size={20} color="red"  />
