    ...
    const handleUpdate = async () => {
      if (!passwordOld) {
        ...
        setError({
          ...
          try {
              setIsLoading(true);
              const res = await axiosClient(`${BASE_URL}/v1/user/changePassword/${user?.info?._id}`,
                {
                  method: 'post',
                  data: formData
                }
                ...
                <View className='mt-[10px] border-b-[1px] border-solid border-[#d3cece]'>
                    ...
                    <View className='flex-row justify-center items-center'>
                        <View className='flex-1'>
                            <TextInput value={passwordOld} onChangeText={val => {
            setError({
              status: false,
              message: ''
            }); setErrorPasswordOld(false); setPasswordOld(val)}} className='text-[16px]' selectionColor="#333" />
                        </View>
                        {
                            errorPasswordOld &&
                ...
                <View className='mt-[10px] border-b-[1px] border-solid border-[#d3cece]'>
                    ...
                    <View className='flex-row justify-center items-center'>
                        <View className='flex-1'>
                            <TextInput value={passwordNew} onChangeText={val => {setError({
              status: false,
              message: ''
            }); setErrorPasswordNew(false); setPasswordNew(val)}} className='text-[16px]' selectionColor="#333" />
                        </View>
                        {
                            errorPasswordNew &&
                            <Feather className='' name="alert-circle" size={20} color="red"  />
                ...
                <View className='mt-[10px] border-b-[1px] border-solid border-[#d3cece]'>
                    ...
                    <View className='flex-row justify-center items-center'>
                        <View className='flex-1'>
                            <TextInput value={rePasswordNew} onChangeText={val => {setError({
              status: false,
              message: ''
            }); setErrorRePasswordNew(false); setRePasswordNew(val)}} className='text-[16px]' selectionColor="#333" />
                        </View>
                        {
                            errorRePasswordNew &&
                            <Feather className='' name="alert-circle" size={20} color="red"  />
